...
- **Trusted adaptive-start extension:** `startQuizSession` consumes a current compatible server-created `adaptiveAssignments` projection and its selected `bankId`; the caller cannot select or override a bank. It verifies trusted lineage, active bank/content compatibility, and question-answer-key completeness before emitting a deterministic five-question form. Missing, stale, inactive, incompatible, or non-runtime assignments fall back to the active Easy bank. This is the final delivery boundary for the U8 adaptive assignment, not another client-side policy implementation.
...
#### U9 parent-access extension

The dashboard's safe-projection boundary now uses a self-service, server-owned invitation path in addition to the protected supervisor exception/revocation path. An authenticated student may create an opaque, expiring invitation for a parent email; Firebase Email Link verifies that email; the parent accepts or declines through a separate named Firebase session; and callable Functions revalidate email match, expiry, verifier, and single-use state before creating the canonical active `parentLinks/{parentId}_{studentId}` record. The student cannot choose a parent UID or write a link. Parent sign-out closes only the named parent session and returns to the preserved student Settings context. Firebase Auth handles parent sign-in and password-reset email delivery; the AI runtime and projections never receive invitation secrets, credential material, or raw invitation/audit documents. `docs/architecture/logic-oasis-firestore-database-schema.md` remains an older parent-access snapshot and is not current evidence for this invitation/audit contract until it is reconciled separately.
