...
title: Logic Oasis FYP1 Prototype Development - Plan
type: feat
date: 2026-07-05
created: 2026-07-05
updated: 2026-08-11
...
Complete a defensible FYP1 prototype in which a Year 4-6 student receives questions without answer keys, submits each response for server-authoritative correctness, receives authored question-specific guidance after a wrong answer, finalizes a trusted quiz attempt, and automatically triggers mastery, weakness prediction, SHAP explanation, and adaptive next-bank selection. The resulting safe signals power a parent Progress Map that distinguishes understanding, practice effort, and Mutual Aid participation without exposing model internals. For Objective 3, complete one AI-supported collaborative-learning feature: a Q&A Forum with an automatically invoked, versioned Naive Bayes explanation-quality pipeline. The final FYP1 architecture also replaces static Oasis restoration imagery with a persistent, portrait-oriented Flame game scene whose rewards and world mutations are backend-authoritative.
...
| S7 | Parents need evidence-led insight, not an explanation of AI calculations. | Replace the raw technical dashboard with a Progress Map: a primary Understanding focus, a Practice Effort weekly rhythm, a count-only Mutual Aid activity view, and one gentle action grounded in visible evidence. |
| S8 | A wrong-answer message must teach the exact reasoning steps without expanding into an adaptive tutor. | Return bilingual, server-authored, question-specific guidance steps only after backend validation. In FYP1 the learner reviews the steps and moves to the next question; retry, second-failure recovery, similar fresh questions, and AI Guard conversation remain FYP2. |
...
|---|---|---|
| Question-bank switching | Implemented for the Year 4 `read_write_numbers` slice: three server-seeded banks exist and `startQuizSession` consumes only a compatible server-created `adaptiveAssignments` record. A client-supplied bank cannot override it; invalid or absent assignment safely falls back to an active Easy bank. Forms are deterministic five-question rotations. | Extend only through validated server-seeded banks and the existing guarded-policy/runtime contract; do not reintroduce client bank selection. |
| Immediate quiz feedback | The current secure quiz flow returns one generic server explanation after validation. | Keep authenticated backend validation, but return structured bilingual guidance steps only after an incorrect response; show the steps before moving on, without revealing the correct answer or adding a learner retry branch. |
| AI runtime | Implemented and live verified as the U8 finalized-attempt runtime with safe status, mastery, and adaptive-assignment projections. The active XGBoost path is the immutable developer-released FYP1 controlled demonstration. | Preserve the controlled-demo evidence-mode boundary: it demonstrates runtime mechanics but is not real-student accuracy, learning-improvement, or model-superiority evidence. Any future `real_evaluated` release remains separately governed. |
| Parent access and dashboard | Implemented as a separate Firebase parent session with an active server-owned `parentLinks/{parentId}_{studentId}` relationship. Students invite a parent email; the parent verifies and accepts from that email before gaining safe linked-child dashboard access. | Keep parent invitation, acceptance, link/revocation, and password recovery inside Firebase Auth/callable boundaries. Never restore client-visible parent credentials or direct client link writes. |
| Parent information design | The dashboard exposes mastery percentages, server/evidence labels, controlled-demo wording, and a compact participation count. | Present a parent-friendly Progress Map: named learning focus with evidence and action, a server-derived weekly practice aggregate, and count-only Mutual Aid moments. Do not show raw model, SHAP, server, or controlled-demo wording in the main parent experience. |
...
2. One authenticated server-owned quiz session that returns answer-free questions, validates every response, provides detailed question-specific guidance after each wrong answer, and finalizes one real attempt with atomic per-question evidence.
...
4. Parent Dashboard reads only safe derived projections and translates them into an Understanding, Practice Effort, and Mutual Aid Progress Map with one gentle action.
...
8. Wrong-answer guidance never exposes the correct option, opens a learner retry, or records hint/retry telemetry in FYP1.
...
### Approved FYP1 Feedback and Parent Progress Refinements

**S7 — Parent Progress Map.** The parent dashboard must lead with one named Understanding focus only when a valid safe subtopic projection supports it. It must show a plain-language status, limited evidence, and one practical parent action. Practice Effort must use a server-owned Asia/Kuala_Lumpur weekly aggregate rather than raw attempts or a mastery bar. Mutual Aid must use only count-only, windowed participation summaries and never infer a child's confidence, expose peer content, or treat a missing projection as zero activity. When evidence is insufficient, the map must say so neutrally instead of ranking a weak area or drawing a trend.

**S8 — Guided wrong-answer feedback.** Active question banks must contain ordered bilingual guidance steps that are validated before use. The server returns those steps only after it validates an incorrect selection; the student can review them and then continue to the next question or finish the quiz. Guidance is authored content, not generative AI, and does not create hint, step-completion, retry, or mastery telemetry in FYP1.

**FYP2 boundary.** A second-incorrect branch, full worked solution, similar fresh question from the adaptive bank, learner retry on the same question, and the conversational AI Guard persona are all deferred. Secure network submission retry remains in scope because it protects the existing authoritative-validation contract, not because it is a learning retry.
...
- server-only `answerIndex`, correct-response acknowledgement, and aligned ordered `guidedSteps`/`guidedStepsBm` in the matching `questionAnswerKeys` document; each step teaches the method without stating the final answer
...
- `Moved Moderate -> Easy temporarily because recent mastery declined; the student will receive a different five-question form from the Easy bank where the pool permits, avoiding recently seen questions.`

Do not hardcode `80% = move up` as the main decision. Score remains one feature, while mastery history, question difficulty, response time, and uncertainty influence the decision.
...
- Parents see parent-friendly status bands, concrete evidence, and a gentle next action. Raw model probabilities, SHAP terms, server labels, and controlled-demonstration wording stay outside the main parent experience.
...
3. For each question, Flutter calls `submitQuizResponse` with `sessionId`, `questionId`, `selectedIndex`, sequence index, client-observed response time, the legacy `hintCount`, and an idempotency token. FYP1 has no learner-triggered hint interaction, so `hintCount` remains `0` and is not a model feature.
4. The backend verifies session ownership, expected question/order, content version, and first-submission state; reads the matching server-only `questionAnswerKeys` document; writes one immutable `questionResponses/{responseId}` record with trusted correctness, `responseTimeQuality: client_reported_unverified`, and `hintTelemetryStatus: not_supported`; and returns either a brief correct confirmation or ordered bilingual guidance steps after an incorrect answer. Incorrect feedback must not contain an answer index, correct option, or final-answer wording.
5. Flutter displays the returned feedback. An incorrect response shows the authored steps in sequence, then moves to the next question; it does not reopen the choices or branch on a second failure. If the call fails, the response remains `pending_validation`, is retried idempotently, and no local answer key, correctness, or guidance is shown.
...
- Runtime inference requires an active server-only `modelRegistry` record with immutable developer release metadata, evaluation-report hash, and compatible package, feature-schema, artifact, ranking-policy, and adaptive-policy hashes. Any missing/inactive/mismatched record produces the declared BKT/rule fallback. Signed release/revocation infrastructure is deferred to production hardening.
...
| `questionAnswerKeys` | Server-only source for authoritative correctness, correct-response acknowledgement, and wrong-answer guidance. Client reads are denied. | `questionId`, `answerIndex`, `correctFeedback`, `correctFeedbackBm`, ordered `guidedSteps`, ordered `guidedStepsBm`, `contentVersion`, `isActive`. |
...
| `parentPracticeSummaries/{studentId}` | Backend-derived weekly safe parent projection updated with trusted attempt finalization. | `studentId`, Malaysia-local `weekStart`, bounded daily completion counts, `activeDayCount`, `completedPracticeCount`, optional `previousWeekCompletedPracticeCount`, `updatedAt`. It contains no attempt, response, question, answer, or model identifiers. |
| `forumParticipationSummaries/{studentId}` | Backend-derived, count-only safe parent projection. | `studentId`, Malaysia-local `weekStart`, windowed `questionsPostedCount`, `answersSubmittedCount`, `acceptedAnswersCount`, `helpfulReceivedCount`, `lastParticipationAt`, `updatedAt`. It never contains forum text, question/answer IDs, peer identities, moderation fields, or model output. |
...
**Approach:** Define Easy/Moderate/Hard bank metadata, 8-10 validated bilingual questions per bank, deterministic five-question form rotation, recent-exposure filtering, and content versions. Migrate authoritative `answerIndex`/explanations out of Flutter-bundled runtime content into server-seeded `questionAnswerKeys`; deny client reads. Client-readable question models and Firestore documents contain prompts/options only. `startQuizSession` accepts no client bank-selection authority: it uses only a current compatible `runtime_callable` adaptive assignment and otherwise falls back safely to an active Easy bank. Existing local content may remain as a migration or test fixture only after authoritative answers are removed.

**Test scenarios:** Each bank has 8-10 unique active questions; generated forms contain five valid questions in deterministic rotation; recent questions are avoided when alternatives exist; malformed bank/question links fail validation; a client-provided bank cannot override a compatible adaptive assignment; absent, stale, inactive, or incompatible assignments fall back to Easy; client reads of `questionAnswerKeys` are denied; built Flutter assets and runtime question payloads contain no authoritative answer index or explanation.

**Verification:** One subtopic can produce distinct Easy/Moderate/Hard server-selected forms and all content/answer-key references validate.
...
| Client access | Students and linked parents read only `studentAiStatuses`, `adaptiveAssignments`, and `subtopicMastery`; active linked parents may additionally read their child's server-owned `parentPracticeSummaries` and count-only `forumParticipationSummaries`; all raw jobs, model runs, SHAP arrays, hashes, paths, and errors are server-only. |
...
**Files:** `lib/shared/models/ai_diagnosis.dart`, `lib/shared/models/forum_participation_summary.dart`, `lib/shared/repositories/learning_repository.dart`, `lib/shared/services/ai_status_service.dart`, `lib/shared/services/parent_firebase_session.dart`, `lib/shared/services/parent_link_invitation_service.dart`, `lib/shared/services/parent_invitation_link_service.dart`, `lib/shared/state/app_state.dart`, `lib/features/quiz/result_page.dart`, `lib/features/home/home_page.dart`, `lib/features/parent_dashboard/parent_dashboard_page.dart`, `lib/features/settings/parent_access_page.dart`, `lib/features/settings/parent_invitation_page.dart`, `lib/features/settings/parent_invitation_accept_page.dart`, `functions/main.py`, `functions/parent_link_admin.py`, `functions/parent_link_invitation.py`, `functions/parent_link_email_delivery.py`, `tools/bootstrap_parent_link_admin.py`, `tools/deploy_parent_invitation_iam.py`, `firestore.rules`, `firebase.json`, `web/.well-known/assetlinks.json`, `docs/architecture/logic-oasis-firestore-database-schema.md`, `test/ai_diagnosis_test.dart`, `test/result_page_test.dart`, `test/parent_access_page_test.dart`, `test/parent_invitation_page_test.dart`, `test/parent_invitation_link_service_test.dart`, `test/forum_participation_summary_test.dart`, `functions/tests/test_parent_link_admin.py`, `functions/tests/test_parent_link_invitation.py`, `functions/tests/test_parent_link_email_delivery.py`, and `functions/tests/test_parent_link_rules.py`.

**Approach:** Show immediate quiz results, `analysis in progress`, completed/fallback/failed states, and the assigned next mission for the student. Keep any model-derived detail inside safe student status; U14 owns the parent-language translation. Seeded AI remains test/fallback content, not the normal evidence path. The only authoritative relationship is `parentLinks/{parentId}_{studentId}` with `status: active`. Normal creation is self-service but server-owned: an authenticated student sends an email-bound invitation; Firebase Email Link verifies the invited parent email; the parent accepts or declines through a separate Firebase parent session; and callable Functions validate expiry, one-time verifier, matching email, and replay state before creating the active link and immutable `parentLinkAudits` record. The parent can unlink their own active relationship; the student cannot choose a parent UID, inspect invitation secrets, or write/revive a link. `manageParentLink`/`revokeParentLink` and `tools/bootstrap_parent_link_admin.py` remain the supervisor-approved exception/revocation path, using the dedicated parent-link admin and identity-admin service accounts with refresh-token revocation. The default Firebase app keeps the student signed in while the named parent app opens the dashboard and returns to the original Settings context; Firebase Auth email/password sign-in and its email reset flow replace any local parent password/OTP design. U10 backend code owns a count-only windowed `forumParticipationSummaries/{studentId}` projection containing a Malaysia-local `weekStart`, `questionsPostedCount`, `answersSubmittedCount`, `acceptedAnswersCount`, `helpfulReceivedCount`, `lastParticipationAt`, and `updatedAt`; it contains no forum text, question/answer IDs, peer identities, moderation detail, or model output. Trusted quiz finalization owns the separate `parentPracticeSummaries/{studentId}` weekly aggregate. Firestore Rules use the exact active link to permit a parent read only of the linked student's `studentAiStatuses`, `adaptiveAssignments`, `subtopicMastery`, `parentPracticeSummaries/{studentId}`, and `forumParticipationSummaries/{studentId}`. Direct parent reads of invitations, link audits, raw attempts, AI jobs/runs, SHAP, model registry, forum text, or another student's projections are denied.

**Test scenarios:** Processing does not block results; completed records refresh both student and parent surfaces; fallback/failure is transparent; stale runs are not shown as latest; technical weak labels are translated into supportive child-facing language; a student can create a masked email-bound invitation; only the verified matching parent email can accept it once before expiry; wrong email, replay, expired invitation, unrelated parent, cross-student parent, revoked link, and direct Firestore access are denied; parent sign-out returns to the still-signed-in student's Settings context; Firebase password-reset requests disclose no account existence; an active linked parent reads each declared safe child projection, their child's server-derived practice aggregate, and only their child's count-only forum participation summary; bootstrap exception/revoke produces an immutable audit record, invalidates revoked tokens, and rejects missing/revoked claims; raw invitation/link audit/AI/forum data remains unreadable to all parents.

**Verification:** The manual demo traces one attempt ID from quiz submission to parent explanation and next bank, then proves a disposable student invitation -> verified parent acceptance -> parent dashboard -> parent sign-out -> retained student Settings flow.

### U12. Secure Self-Service Parent Invitation and Isolated Parent Session

**Goal:** Let a student invite a real parent account safely, while preserving the student session and exposing only the linked child's approved dashboard projections.

**Requirements:** S2. **Dependencies:** U9 and Firebase Authentication Email/Password plus Email Link sign-in.

**Files:** `functions/main.py`, `functions/parent_link_invitation.py`, `functions/parent_link_email_delivery.py`, `functions/parent_link_context.py`, `functions/parent_link_admin.py`, `functions/tests/test_parent_link_invitation.py`, `functions/tests/test_parent_link_email_delivery.py`, `functions/tests/test_parent_link_context.py`, `functions/tests/test_parent_link_rules.py`, `lib/app/logic_oasis_app.dart`, `lib/features/settings/settings_page.dart`, `lib/features/settings/parent_access_page.dart`, `lib/features/settings/parent_invitation_page.dart`, `lib/features/settings/parent_invitation_accept_page.dart`, `lib/features/parent_dashboard/parent_dashboard_page.dart`, `lib/shared/services/parent_firebase_session.dart`, `lib/shared/services/parent_link_invitation_service.dart`, `lib/shared/services/parent_invitation_link_service.dart`, `test/parent_access_page_test.dart`, `test/parent_invitation_page_test.dart`, `test/parent_invitation_link_service_test.dart`, `firestore.rules`, `firebase.json`, `web/.well-known/assetlinks.json`, `tools/deploy_parent_invitation_iam.py`, and `docs/architecture/logic-oasis-firestore-database-schema.md`.

**Approach:** Keep Firebase Authentication as the sole credential authority. The student creates an opaque, expiring invitation for a normalized parent email; the dedicated invitation runtime stores only server-owned invitation state, email HMAC/verifier material, and audit metadata. Before Email Link delivery, enforce one active invitation per student/normalized-recipient pair, a ten-minute resend cooldown, and at most three new invitations per student in 24 hours; return the same privacy-safe acknowledgement for a throttled request. Email delivery uses Firebase Email Link with a Hosting continuation URL. The parent opens the verified link, authenticates in the isolated named Firebase app, and accepts or declines through a callable that rechecks the verified email, invitation expiry, one-time state, and active relationship. The callable creates/revokes the single canonical `parentLinks/{parentId}_{studentId}` document and append-only `parentLinkAudits`; on link creation it initializes the current Malaysia-week zero summaries needed by U14 when no activity has yet been recorded. Direct Firestore reads/writes of invitation/audit/link data are denied. The parent dashboard gets active-child context only through a callable-safe projection. Parent sign-out closes only the named parent app and returns to the preserved student Settings route. Password recovery uses Firebase's email reset delivery and must not reveal whether an address has an account. Hosting/App Links and the narrow invitation service-account/Secret Manager configuration are deployment prerequisites, not client configuration.

**Test scenarios:** A student can send an invitation without selecting a parent UID; repeated and cross-recipient requests respect the active-invitation, cooldown, and daily quota without revealing private state; a matching verified email accepts once and creates one active canonical link plus current-week zero parent summaries; wrong email, expired/replayed/declined invitation, unlinked parent, revoked link, foreign child, and direct Firestore access fail; parent sign-in/sign-out leaves the default student session and settings navigation intact; reset requests always return the same safe acknowledgement; callback link parsing accepts Firebase's nested email-link continuation and rejects malformed/foreign routes; Rules deny invitation secrets, link audits, raw attempts, jobs, runs, SHAP, and model data; disposable deployment verification cleans up all test accounts/links/invitations.
...
**Approach:** Implement post/filter/answer/reasoning/helpful/accept/report flows with explicit authority: authors create/edit only their eligible posts, only a question author may accept a different learner's answer, each eligible learner has one helpful mark per answer, report/block writes are scoped to the reporter, and trusted moderation/runtime alone changes moderation state. Keep labels in a pseudonymized, de-identified offline dataset and rubric; train/evaluate one vectorizer-classifier pipeline; trigger inference automatically through `functions/main.py` into `functions/forum_runtime.py`, deployed with Python `service_account="logic-oasis-forum-runtime@logic-oasis-fyp.iam.gserviceaccount.com"` and only project `roles/datastore.user` plus `roles/logging.logWriter`; the deployer receives scoped `roles/iam.serviceAccountUser` only on that runtime identity. The runtime writes deterministic `forumAiJobs/{answerId}` (`queued -> processing -> completed|fallback|failed`), immutable `forumAiRuns/{answerId}`, a safe answer-feedback state, and the count-only `forumParticipationSummaries/{studentId}` projection. Each counter is derived transactionally and idempotently from its server-stamped event time: question creation, answer creation, another learner's acceptance, or a helpful mark, respectively. Convert that event timestamp to the Asia/Kuala_Lumpur `weekStart` before incrementing so delayed/retried processing never moves an event into another week. The projection contains no forum text, IDs, peers, moderation data, or classifier output. It preserves uncertain and fallback states without exposing private answer text to parents. U10 ends at verified forum and advisory Naive Bayes behavior: it does not create wallet/ledger entries or award Mutual Aid Energy; the one-time accepted-answer Energy reward is owned by G6b and verified with the Oasis/U11 reward gates.

**Test scenarios:** Complete forum flow persists questions/answers, explanation-quality feedback, and count-only windowed participation summaries without an Energy reward; foreign authors cannot accept, self-accept, duplicate-helpful, or mutate another learner's content; week rollover, cross-week acceptance/helpful marks, and delayed/retried events use the originating server timestamp and deterministic event ID; answer-only text receives revision guidance; sufficient text publishes; uncertain output remains editable; duplicate trigger is idempotent; job/run/output IDs and `queued -> processing -> completed|fallback|failed` transitions are deterministic; the forum runtime identity is explicitly bound and rejects default/broad identities; bilingual examples are evaluated separately where data permits; uncalibrated probability is not labelled confidence.
...
### U13. Structured Wrong-Answer Guidance

**Goal:** Replace the generic wrong-answer sentence with bilingual, question-specific learning steps while preserving the server-authoritative quiz contract.

**Requirements:** S8. **Dependencies:** U2, U3-R. **Estimate:** 2-3 days.

**Files:** `firebase_seed/seed_data.json`, `firebase_seed/year4_read_write_question_banks.js`, `firebase_seed/year4_whole_numbers_additional_banks.js`, `firebase_seed/seed_firestore.js`, `firebase_seed/validate_question_banks.js`, `functions/main.py`, `functions/quiz_session.py`, `lib/shared/models/question_response.dart`, `lib/shared/services/quiz_session_service.dart`, `lib/features/quiz/quiz_page.dart`, `lib/l10n/app_en.arb`, `lib/l10n/app_ms.arb`, `functions/tests/test_quiz_session.py`, `functions/tests/test_attempt_validation.py`, `functions/tests/test_start_quiz_session_adaptive.py`, `test/quiz_session_service_test.dart`, and `test/quiz_feedback_guidance_test.dart`.

**Approach:** Extend the server-only answer-key contract with non-empty, ordered, bilingual `guidedSteps` content. Validate every active bank's steps during seed/preflight validation, including aligned English/Bahasa Melayu counts and a no-final-answer content review. Keep all guidance out of `startQuizSession` and client-readable question documents. After a validated incorrect response, the callable projects only the ordered guidance steps into `QuestionResponse`; after a correct response, it projects only a short positive confirmation. `QuizPage` presents the incorrect-response steps in sequence, then enables its existing Next Question or Finish Quiz transition. Guidance is transient display state: do not add learner answer retry, step-completion tracking, hint count, fresh-question selection, or AI Guard dialogue.

**Patterns to follow:** `questionAnswerKeys` access boundary in `functions/main.py` and `functions/quiz_session.py`; safe callable parsing in `lib/shared/models/question_response.dart`; pending-validation recovery in `lib/features/quiz/quiz_page.dart`.

**Test scenarios:** An active bank with missing, empty, misaligned bilingual, or answer-revealing guidance fails validation before a session starts; session question payloads and public question documents never contain steps or answer keys; a validated wrong answer returns only its ordered localized steps; a correct answer returns no corrective steps; pending/failed network validation exposes neither correctness nor guidance and uses the existing idempotent secure retry; a duplicate idempotency key returns the same validated guidance without resealing the response; the final-question guidance remains readable before Finish Quiz; the UI offers no answer re-selection, learner retry, second-failure, similar-question, or AI Guard branch.

**Verification:** A real/emulator wrong answer displays its specific bilingual reasoning steps and then progresses normally, while security tests prove that the guidance cannot be obtained before server validation.

### U14. Parent Evidence Progress Map

**Goal:** Replace the technical parent dashboard with a differentiated, evidence-led map of understanding, practice effort, and Mutual Aid participation.

**Requirements:** S7. **Dependencies:** U9, U12. **Integration dependency:** U10 supplies windowed Mutual Aid counts when forum activity is available; its absence must render as unavailable rather than zero. **Estimate:** 3-4 days.

**Files:** `functions/main.py`, `functions/quiz_session.py`, `firestore.rules`, `docs/architecture/logic-oasis-firestore-database-schema.md`, `lib/shared/models/trusted_subtopic_progress.dart`, `lib/shared/models/parent_practice_summary.dart`, `lib/shared/models/parent_dashboard_snapshot.dart`, `lib/shared/models/forum_participation_summary.dart`, `lib/shared/repositories/learning_repository.dart`, `lib/shared/state/app_state.dart`, `lib/features/parent_dashboard/parent_dashboard_page.dart`, `lib/l10n/app_en.arb`, `lib/l10n/app_ms.arb`, `functions/tests/test_quiz_session.py`, `functions/tests/test_parent_link_rules.py`, `test/learning_repository_test.dart`, `test/parent_dashboard_linked_child_test.dart`, `test/parent_dashboard_time_test.dart`, and `test/forum_participation_summary_test.dart`.

**Approach:** Preserve the existing linked-parent safe-projection boundary. Parse safe subtopic-mastery records into `TrustedSubtopicProgress`. A named Understanding focus is eligible only when its parsed record is valid, has `evidenceLevel: established`, has a positive `observationCount`, and is no older than 14 days. Select the lowest mastery probability; break an exact tie by newest `updatedAt`, then stable `subtopicId`. Otherwise render the neutral insufficient-evidence state and do not rank a weakness. Add a server-written `parentPracticeSummaries/{studentId}` projection updated atomically when a trusted quiz attempt is finalized. The projection stores an Asia/Kuala_Lumpur `weekStart`, bounded daily completion counts, active-day/weekly totals, prior-week aggregate where available, and no question, response, or attempt identifiers. U12 initializes a current-week zero summary on a new active parent link; a missing or stale-window projection remains unavailable rather than being inferred as zero. Keep current-window Mutual Aid counters and `weekStart` in the existing count-only forum projection; do not label lifetime counts as weekly activity. Choose the one parent action deterministically: eligible Understanding focus first; otherwise valid zero/low Practice Effort; otherwise a neutral Mutual Aid invitation; otherwise say that more activity is needed before a recommendation. Mutual Aid encouragement never describes a child as shy, unwilling, or deficient. Render Understanding as the primary full-width focus/evidence/action card, Practice Effort as a weekly rhythm rather than a mastery bar, and Mutual Aid as a count-only community-moments view. Give each surface explicit loading, no-active-child, unavailable/read-failure-with-retry, and current-zero states; clear old child data before a switch completes. Remove raw server, model, SHAP, evidence-level, controlled-demonstration, and private explanation-reason wording from the parent surface. Use ordered semantic guidance, explicit Next/Finish labels, non-colour correctness cues, logical focus order, and text-scaling-safe layouts for the new quiz and parent components.

**Patterns to follow:** linked-child loading and stale-selection protection in `lib/features/parent_dashboard/parent_dashboard_page.dart`; safe snapshot assembly in `lib/shared/repositories/learning_repository.dart`; count-only privacy contract in `lib/shared/models/forum_participation_summary.dart`.
...
**Requirements:** S1-S8. **Dependencies:** U2, U3, U3-R, U4-U10, U12, U13, U14, G1, G2, G3, G4, G5a, G6a, G5b, G6b, G7. **Estimate:** 3-5 days.

**Files:** `firestore.rules`, `firebase.json`, `test/`, `test/oasis/`, `functions/tests/`, `ai_pipeline/tests/`, `tools/tests/`, `docs/logic_oasis_feature_implementation_explanation.md`, `docs/plans/logic-oasis-interactive-game-summary-plan.md`, `docs/architecture/logic-oasis-firestore-database-schema.md`, `ai_pipeline/reports/model_comparison.md`.
...
**Test scenarios:** Unauthorized answer-key, wallet, ledger, world, raw-attempt, and parent-summary writes fail; callable functions reject unauthenticated/foreign/expired sessions; students cannot award rewards, edit AI outputs, or mutate another student's world; all unit/integration gates pass; seed rows are absent from final metrics; the complete manual flow works from login through trusted guided wrong-answer feedback, parent Progress Map evidence, Q&A evidence, and Oasis restoration.
...
flowchart LR
    ...
    U12 --> U14
    U8 --> U10
    U10 -. windowed Mutual Aid counts .-> U14
    U1 --> G1 --> G2
    U3 --> G3
    G1 --> G3 --> G4
    G2 --> G4 --> G5a
...
The combined core-FYP1 estimate is 57.5-93 working days plus 2-4 Stage 1 asset-production days, excluding approval/data-collection waiting time. U13 and U14 add 5-7 days and may begin after their stated dependencies; U14's Mutual Aid empty state does not wait for U10, but its populated weekly card does. Conditional G5c Stage 2 presentation work adds 1-2 engineering days plus 4-8 asset-production days only when separately approved. Some game configuration/art work can proceed in parallel with AI/data work, but this does not reduce the required evidence gates. U8 scaffolding may start only after U3-R verifies the secure sequence/telemetry contract, and G1/G2 may start after U1; final inference integration cannot complete until U7 promotes the replacement model. If cloud deployment is blocked, the Firebase Emulator must execute the same `functions/main.py` entry point automatically; a manual post-quiz script is not equivalent.
...
| Question-bank validation | Every active bank has valid topic/subtopic/skill/difficulty links, sufficient active questions, no duplicate IDs, and bilingual correct-feedback and ordered guided-step fields required for the demo. |
...
| Guided immediate feedback | Every accepted answer returns backend-derived correctness; a wrong answer returns only ordered question-specific guidance steps after validation, while a correct answer returns a short confirmation. Failed calls remain pending/retryable and never reveal a local answer key or guidance. |
...
| Parent-link authorization | `parentLinks/{parentId}_{studentId}` is the only parent-child authority. The normal path is an authenticated student-created, email-bound, expiring parent invitation accepted only through the verified matching parent's isolated Firebase session; direct client reads/writes of invitations, links, and audits are denied. The protected identity-admin bootstrap remains the supervisor-approved exception/revocation path and invalidates revoked tokens. Emulator tests prove an active linked parent can read only that student's `studentAiStatuses`, `adaptiveAssignments`, `subtopicMastery`, server-owned `parentPracticeSummaries`, and count-only `forumParticipationSummaries`; wrong-email, replayed, expired, unrelated, cross-student, revoked, self-created, missing-claim, and revoked-token access are denied. |
| Parent Progress Map | Understanding selects a valid safe subtopic focus or renders an insufficient-evidence state; Practice Effort reads only the server-owned Malaysia-week aggregate; Mutual Aid reads only current-window count-only counters. Widget and Rules tests prove distinct visual semantics, linked-child isolation, missing-versus-zero handling, and absence of raw model, AI, server, attempt, answer, peer, and forum-text exposure. |
...
| Manual end-to-end demo | Real login -> server-created quiz session -> wrong answer shows validated detailed steps -> finalized attempt -> automatic learning AI -> server-selected next bank -> parent invitation/verified acceptance -> parent Progress Map -> parent sign-out back to the preserved student Settings context -> Q&A answer -> automatic Naive Bayes decision/feedback completes without local answer keys or seed AI records. |
...
| Guided feedback becomes an answer reveal or generic hint | Author and validate ordered bilingual steps in the server-only answer key; reject malformed or answer-revealing content, return it only after a wrong response, and test the absence of in-question learner retry branches. |
| Parent insight overstates weak evidence or exposes private data | Derive only bounded safe projections, distinguish unavailable from zero activity, require sufficient subtopic evidence before ranking a focus, and retain Firestore Rules tests that deny raw attempts, responses, forum text, and model records. |
...
- A validated wrong response receives only its authored bilingual, question-specific guidance steps; the client receives no answer key, correct option, or final-answer wording.
- The learner can review wrong-answer guidance and then use only the existing Next Question or Finish Quiz transition. FYP1 adds no learner retry, second-failure branch, similar fresh question, or AI Guard conversation.
...
- Parent Dashboard shows an evidence-led Understanding focus, a server-derived Practice Effort rhythm, count-only windowed Mutual Aid moments, and one supportive parent action, not a manually seeded AI record or raw model calculation.
...
- `parentLinks/{parentId}_{studentId}` is the only parent-child authority; active linked parents can read only their child's safe status, assignment, mastery, server-owned practice, and count-only Mutual Aid projections.
...
- Parent-safe weekly practice and Mutual Aid projections use Malaysia-local week boundaries, distinguish unavailable data from recorded zero activity, and do not expose raw attempts, forum content, peer identities, or AI output.
...
- **FYP2-I3. Quiz recovery and AI Guard:** After repeated difficulty, reveal a full worked solution, select a similar fresh question from the adaptive bank, and optionally present the established guidance through a warm conversational persona. This work must define retry scoring, recovery evidence, and tutor guardrails before implementation.
...
|---|---|
| Server-authoritative detailed wrong-answer guidance | U2, U3, U13, U11 |
...
| Parent evidence Progress Map | U9, U10, U12, U14 |
